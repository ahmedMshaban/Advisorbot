...
class AdvisorMain {
    ...
    void printProd();
    /** Find minimum bid or ask for product in current timestep*/
    void printMin(const std::vector < std::string > & tokens);
    /** Find maximum bid or ask for product in current timestep*/
    void printMax(const std::vector < std::string > & tokens);
    /** Compute average ask or bid for the sent product from over the last n timesteps*/
    ...
    void printVolatility(const std::vector < std::string > & tokens);
    /** Predict max or min ask or bid for the sent product for the next timestep */
    void printPredict(const std::vector < std::string > & tokens);
    /** Move to next timestep */
    void step(const std::vector < std::string > & tokens);
    /** print current timestep  index*/
    ...
    bool productNameIsValid(std::string productName,
    ...
    bool commanNameIsValid(std::string commandName,
    ...
    /** Return a predicted min price for a product in the next timestep */
    double getPredictMin(std::string productName, int timestampIndex, std::string productType);
    /** Return a predicted max price for a product in the next timestep   */
    ...
    int currentTimeStep;
    std::vector < std::string > avaliableCommands {
        "help",
        "prod",
        "min",
        "max",
